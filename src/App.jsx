...
/* ═══ BRAINZAP SPLASH ════════════════════════════════════════════════════════
 * App name: BrainZap ⚡🧠
 * Why: "Brain" = knowledge; "Zap" = instant, electric, modern.
 *      Together: getting struck by a bolt of trivia brilliance.
 * Tone: playful, energetic, slightly absurd.
 * ─────────────────────────────────────────────────────────────────────────── */

/* Full-screen centred splash container */
.bz-splash {
...
/* ── Electric arc ring behind the logo ── */
.bz-ring {
  position: relative; width: 180px; height: 180px;
  display: flex; align-items: center; justify-content: center;
}
.bz-ring svg { position: absolute; inset: 0; }
.bz-ring-track { opacity: 0.15; }
.bz-ring-arc   {
  stroke-dasharray: 565; stroke-dashoffset: 565;
  animation: bzArcFill 1.1s cubic-bezier(.4,0,.2,1) 0.2s forwards;
...
/* ── Brain + zap emoji stack ── */
.bz-logo-emoji {
  font-size: 72px; line-height: 1;
  animation: bzBounce 0.7s cubic-bezier(.36,.07,.19,.97) 1.1s both;
...
.bz-wordmark {
  ...
  /* Split gradient: Brain = primary purple, Zap = hot gold */
  ...
  animation: bzSlideUp 0.55s ease 1.4s both;
...
.bz-tagline {
  ...
  animation: bzSlideUp 0.55s ease 1.65s both;
}
...
.bz-hint {
  ...
  animation: bzPulseHint 1.8s ease 2.2s infinite;
}
@keyframes bzPulseHint {
  0%, 100% { opacity: 0.45; }
  50%       { opacity: 1;    }
}

/* ── Floating spark particles ── */
.bz-sparks { position: absolute; inset: 0; pointer-events: none; overflow: hidden; }
.bz-spark  {
  position: absolute;
  width: 6px; height: 6px;
  border-radius: 50%;
  background: var(--primary);
  animation: bzFloat var(--dur) ease-in-out var(--delay) infinite;
  opacity: 0;
}
.bz-spark:nth-child(even)  { background: var(--gold);   width: 5px;  height: 5px; }
.bz-spark:nth-child(3n)    { background: var(--accent);  width: 4px;  height: 4px; }
@keyframes bzFloat {
  0%   { opacity: 0;   transform: translateY(0)    scale(0.5); }
  20%  { opacity: 0.9; transform: translateY(-40px) scale(1);   }
  80%  { opacity: 0.6; transform: translateY(-160px) scale(0.8); }
...
.bz-progress-fill {
  ...
  animation: bzProgress var(--bz-duration, 2600ms) linear forwards;
  transform-origin: left;
}
@keyframes bzProgress {
  from { width: 0%; }
  to   { width: 100%; }
}
`;
...
// BUG FIX: the old code used strokeDashoffset={circ/4} intending to rotate
// the arc start to 12 o'clock, but a *positive* offset instead consumed circ/4
// of the drawn dash — so pct=1 rendered as 75% filled. Proof:
//   visibleArc = (pct * circ) - offset = circ - circ/4 = 3*circ/4 = 75%
//
// Correct approach: rotate the arc element -90° around its own centre.
// This shifts the path's inherent start point (3 o'clock) to 12 o'clock
// without touching the dash lengths, so pct=1 always fills 100%.
function ScoreRing({ score, total, size = 130 }) {
  ...
  return (
    <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`}>
      ...
      {/* Progress arc — rotated so 0% starts at 12 o'clock (top), not 3 o'clock */}
      <circle cx={cx} cy={cy} r={r} fill="none"
      ...
      <text x="50%" y="46%" dominantBaseline="middle" textAnchor="middle"
        style={{fontFamily:"Fredoka One", fontSize:size * 0.22, fill:"var(--text)"}}>
        {score}/{total}
      </text>
      {/* Percentage */}
      <text x="50%" y="70%" dominantBaseline="middle" textAnchor="middle"
        style={{fontFamily:"Fredoka One", fontSize:size * 0.14, fill:"var(--muted)"}}>
        {Math.round(pct * 100)}%
      </text>
    </svg>
  );
...
// ═══════════════════════════════════════════════════════════════════════════════
// BRAINZAP SPLASH SCREEN
// ═══════════════════════════════════════════════════════════════════════════════
// App name: BrainZap ⚡🧠
// Naming logic:
//   "Brain" → trivia, knowledge, thinking hard
//   "Zap"   → instant, electric, modern, surprising
//   Combined: the feeling of getting hit by a bolt of trivia brilliance.
//
// Design choices:
//   • SVG arc that draws itself in (echoes the score ring the user will see)
//   • Logo bounces in with a spring curve — playful, not corporate
//   • Wordmark gradient splits at the B/Z boundary (Brain=purple, Zap=gold)
//   • Floating spark particles reinforce the "electric" theme
//   • Auto-advances after SPLASH_MS ms OR on any tap/click — no "Start" button
//   • Full theme-awareness via CSS custom properties (dark/light just works)
// ─────────────────────────────────────────────────────────────────────────────
const SPLASH_MS = 2800; // auto-advance duration (ms)

// Spark data: {left%, top%, delay, duration} — pre-computed so no Math.random in render
const SPARKS = [
  {l:12, t:75, d:"0.1s", dur:"2.8s"}, {l:25, t:60, d:"0.5s", dur:"3.2s"},
  {l:38, t:80, d:"0.9s", dur:"2.5s"}, {l:52, t:70, d:"0.3s", dur:"3.0s"},
  {l:65, t:65, d:"0.7s", dur:"2.7s"}, {l:78, t:78, d:"0.2s", dur:"3.4s"},
  {l:88, t:55, d:"1.1s", dur:"2.6s"}, {l:18, t:45, d:"1.3s", dur:"3.1s"},
  {l:45, t:40, d:"0.6s", dur:"2.9s"}, {l:70, t:50, d:"1.0s", dur:"3.3s"},
  {l:33, t:30, d:"0.4s", dur:"2.4s"}, {l:58, t:35, d:"0.8s", dur:"3.0s"},
];

function SplashScreen({ onDone }) {
  // Auto-advance
  useEffect(() => {
    const tid = setTimeout(onDone, SPLASH_MS);
    return () => clearTimeout(tid);
  }, [onDone]);

  // The SVG arc circumference for a r=80 circle: 2π×80 ≈ 502.65
  // We use 565 (slightly larger) so the arc slightly overdraws for visual pop
  const arcR    = 80;
  const arcC    = 2 * Math.PI * arcR; // ≈ 502.65
  ...
  return (
    <div
      ...
      style={{"--bz-duration": `${SPLASH_MS}ms`}}
    >
      {/* ── Floating spark particles (purely decorative) ── */}
      <div className="bz-sparks" aria-hidden="true">
        {SPARKS.map((s, i) => (
          <span
            key={i}
            className="bz-spark"
            style={{left:`${s.l}%`, top:`${s.t}%`, "--dur":s.dur, "--delay":s.d}}
          />
        ))}
      </div>

      {/* ── Electric arc ring + brain emoji ── */}
      <div className="bz-ring" aria-hidden="true">
        {/* SVG ring: track + animated arc that draws in */}
        <svg viewBox="0 0 180 180" width="180" height="180">
          ...
          />
          {/* Animated arc — draws clockwise from 12 o'clock */}
          <circle
            cx="90" cy="90" r={arcR}
            fill="none" stroke="var(--primary)" strokeWidth="3"
            strokeDasharray={arcC}        /* total dash = full circumference */
            strokeDashoffset={arcC}       /* starts fully hidden             */
            strokeLinecap="round"
            transform="rotate(-90 90 90)" /* start at 12 o'clock             */
            className="bz-ring-arc"
          />
        </svg>

        {/* Brain emoji sits centred inside the ring */}
        <div className="bz-logo-emoji" aria-hidden="true">🧠</div>
      </div>

      {/* ── Wordmark: "Brain" in primary, "Zap" transitions to gold ── */}
      {/* The gradient is defined in CSS and clips to text */}
      <h1 className="bz-wordmark" aria-label="BrainZap">
        {/* Span trick: split into two spans with aria-hidden so screen readers
            still read "BrainZap" from the parent aria-label */}
        <span aria-hidden="true">Brain</span>
        <span aria-hidden="true" style={{position:"relative"}}>
          Zap
          {/* The ⚡ sits slightly above the 'Z', anchored with absolute pos */}
          <span style={{
            position: "absolute",
            top: "-18px", right: "-12px",
            fontSize: "28px",
            // Re-enable colour — the gradient clip only applies to text glyphs
            WebkitTextFillColor: "initial",
            filter: "drop-shadow(0 0 6px var(--gold))",
            animation: "bzBounce 0.7s cubic-bezier(.36,.07,.19,.97) 1.3s both",
          }} aria-hidden="true">⚡</span>
        </span>
      </h1>

      {/* ── Tagline ── humorous, on-brand */}
      <p className="bz-tagline">Lightning trivia. Zero fluff. All brain.</p>

      {/* ── Tap hint ── */}
      <p className="bz-hint" aria-live="polite">Tap anywhere to start</p>

      {/* ── Auto-progress bar at very bottom ── */}
      <div className="bz-progress-wrap" aria-hidden="true">
        <div className="bz-progress-fill"/>
      </div>
    </div>
  );
}
...
// ═══════════════════════════════════════════════════════════════════════════════
const SCREENS = {
  SPLASH:  "splash",   // BrainZap intro — shown once on first load
  HOME:    "home",
  SETUP:   "setup",
  GAME:    "game",
  RESULTS: "results",
...
export default function App() {
  ...
  // Start on SPLASH; SplashScreen auto-advances (or tap) to HOME
  ...
  return (
    <>
      ...
      {/* ── BrainZap splash — shown once, then replaced by HOME ── */}
      {screen === SCREENS.SPLASH && (
        <SplashScreen onDone={() => setScreen(SCREENS.HOME)}/>
      )}
